...
/*! \typedef typedef char status_t
 *  \brief Type definition for status.
 *
 *  The status_t type is used as parameter for the
 *  rot_get_status() function.
 *
 *  The status reflects the current operating mode of the rotator according to
 *  S - Stopped / Idle
 *  C - Calibration in process
 *  O - Offline (not in Mode 0)
 *  R - Running (in motion)
 *  E - Error
 */
...
struct rot_caps {
  ...
  int (*calibrate)(ROT *rot);
  int (*get_status)(ROT *rot, status_t *status);
...
extern HAMLIB_EXPORT(int) rot_calibrate HAMLIB_PARAMS((ROT *rot));
extern HAMLIB_EXPORT(int) rot_get_status HAMLIB_PARAMS((ROT *rot, status_t *status));
