...
/**
 * Function by Matt Poole
 * \brief command rotator to calibrate itself
 * \param rot	The rot handle
 *
 *  Commands the rotator to calibrate itself
 *
 * \return RIG_OK if the operation has been sucessful, otherwise
 * a negative value if an error occured (in which case, cause is
 * set appropriately).
...
int HAMLIB_API rot_calibrate (ROT *rot)
{
	const struct rot_caps *caps;
	...
	if (caps->calibrate == NULL)
		return -RIG_ENAVAIL;

	return caps->calibrate(rot);
}

/**
 * Function by Matt Poole
 * \brief get the status of rotator
 * \param rot	The rot handle
 * \param status	The location where to store the current azimuth
 *
 *  Retrieves the current status of the rotator.
 *
 * \return RIG_OK if the operation has been sucessful, otherwise
 * a negative value if an error occured (in which case, cause is
 * set appropriately).
...
int HAMLIB_API rot_get_status (ROT *rot, status_t *status)
{
	const struct rot_caps *caps;
	...
	if (caps->get_status == NULL)
		return -RIG_ENAVAIL;

	return caps->get_status(rot, status);
}
