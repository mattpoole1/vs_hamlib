...
struct test_table test_list[] = {
	...
	{ 'p', "get_pos",   get_position,   ARG_OUT, "Azimuth", "Elevation" },
        { 'F', "calibrate", calibrate,      ARG_NONE, },
        { 't', "get_status",get_status,     ARG_OUT, "Status" },
...
void list_models()
{
	...
	printf(" Rig #  Mfg                    ModelMatt                   Version         Status\n");
...
declare_proto_rot(get_status)
{
        ...
        retval = rot_get_status(rot, &status);
        if (retval != RIG_OK)
                return retval;
        if ((interactive && prompt) || (interactive && !prompt && ext_resp))
                fprintf(fout, "%s: ", cmd->arg1);
        fprintf(fout, "%c%c", status, resp_sep);
        return retval;
}
